    ...
    public partial class ModelMaster : Form
    {
        ...
        public void FillGrid()
        {
            ...
            foreach (DataRow dr in _bal.SelectModel().Rows)
            {
                ...
                dgv_ModelMaster.Rows[j].Cells["CCD_Value"].Value = dr["CCD_Value"];
        ...
        {
            _bal.AddModel(txt_shim.Text, dtp_date.Text, txt_masterValue.Text, txt_CCD_value.Text, txt_tolerence_value.Text);
            Reset();
        }
        ...
        private void bttn_update_Click(object sender, EventArgs e)
        {
            try
            {
                _bal.UpdateModel(txt_id.Text, txt_shim.Text, dtp_date.Text, txt_masterValue.Text,txt_CCD_value.Text, txt_tolerence_value.Text);
                Reset();
            }
            catch (Exception ex)
            {
        ...
        private void dgv_ModelMaster_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txt_masterValue.Text = row.Cells["master_value"].Value.ToString();
                txt_CCD_value.Text = row.Cells["CCD_Value"].Value.ToString();
        ...
        public void Reset()
        {
            ...
            txt_shim.ResetText();
            dtp_date.ResetText();
            txt_masterValue.ResetText();
            txt_tolerence_value.ResetText();
            txt_CCD_value.ResetText();
            FillGrid();
        }
       ...
       && (e.KeyChar != '.') && (e.KeyChar != '-'))
        ...
        private void txt_CCD_value_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
