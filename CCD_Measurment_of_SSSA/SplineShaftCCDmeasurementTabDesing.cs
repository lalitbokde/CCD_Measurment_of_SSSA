...
namespace CCD_Measurment_of_SSSA
{
    public partial class SplineShaftCCDmeasurementTabDesing : Form
    {
        ...
        private void bttn_checkshims_Click(object sender, EventArgs e)
        {
            ...
            shim = Math.Abs(Convert.ToDouble(txt_shimsRequired_page2.Text) * 1000);
        ...
        private void bttn_DialGaugeReading_Click(object sender, EventArgs e)
        {
            ...
            shim = Math.Abs(Convert.ToDouble(txt_shimsRequired_page2.Text));
            if (((Convert.ToDouble(txt_shimsRequired_page2.Text) * 1000) - (Convert.ToDouble(txt_dailgaugereading_verification_page3.Text) * 1000)) < Convert.ToDouble(shim_verification_tolerence) * 1000)
            {
        ...
        public void DrivenGaugereadeingCheck()
        {
            try
            {
                if (txt_shimsRequired_page2.Text != "" && txt_dailgaugereading_verification_page3.Text != "")
                {
                    shim = Math.Abs(Convert.ToDouble(txt_shimsRequired_page2.Text));
                    if (Math.Abs(((Convert.ToDouble(txt_shimsRequired_page2.Text) * 1000) - (Convert.ToDouble(txt_dailgaugereading_verification_page3.Text) * 1000))) < Convert.ToDouble(shim_verification_tolerence) * 1000)
                    {
                        Verified.Visible = true;
                        NotVerified.Visible = false;
        ...
        private bool VerifyAll()
        {
            ...
            if (String.IsNullOrEmpty(txt_spline_Shaft_Etched_Value_page2.Text))
            {
                lbl_spline_Shaft_Etched_Value_page2.ForeColor = Color.Red;

                verifyAll = false;
            }
            else
            {
                lbl_spline_Shaft_Etched_Value_page2.ForeColor = Color.Black;
            }


            if (String.IsNullOrEmpty(txt_BH_From_OC_To_IR_page2.Text))
            {
                lbl_BH_From_OC_To_IR_page2.ForeColor = Color.Red;

                verifyAll = false;
            }
            else
            {
                lbl_BH_From_OC_To_IR_page2.ForeColor = Color.Black;
            }

            if (String.IsNullOrEmpty(txt_shimsRequired_page2.Text))
            {
            ...
            if (String.IsNullOrEmpty(txt_spline_Shaft_Etched_Value_page3.Text))
            {
            ...
            if (String.IsNullOrEmpty(txt_BH_From_OC_To_IR_page3.Text))
            {
                lbl_BH_From_OC_To_IR_page3.ForeColor = Color.Red;

                verifyAll = false;
            }
            else
            {
                lbl_BH_From_OC_To_IR_page3.ForeColor = Color.Black;
            }

            if (String.IsNullOrEmpty(txt_shimsRequiredDrivenVerify.Text))
            {
        ...
        private void bttn_saveAll_Click(object sender, EventArgs e)
        {
            if (VerifyAll())
            {
                ...
                _bal.AddPTOMeasurementData(cmb_selectmodel.SelectedValue.ToString(), txt_masterValue_page2.Text, txt_dailgaugereading_page2.Text, txt_ActualReading_page2.Text, txt_CCDValue_page2.Text, txt_spline_Shaft_Etched_Value_page3.Text, txt_BH_From_OC_To_IR_page3.Text, txt_shimsRequiredDrivenVerify.Text, total_shim_required, txt_dailgaugereading_verification_page3.Text, txt_username.Text, DateTime.Now);
                FillGrid();
                Masterresetall();
        ...
        private void Masterresetall()
        {
            ...
            txt_spline_Shaft_Etched_Value_page2.ResetText();
            txt_spline_Shaft_Etched_Value_page3.ResetText();

            txt_BH_From_OC_To_IR_page2.ResetText();
            txt_BH_From_OC_To_IR_page3.ResetText();
            txt_dailgaugereading_page2.Text = "0";
            txt_ActualReading_page2.ResetText();
            ...
            txt_shimsRequired_page2.ResetText();
            LB_Total_shims.Items.Clear();
            Verified.Visible = false;
            NotVerified.Visible = false;
        }
        ...
        private void FillGrid()
        {
            ...
            foreach (DataRow dt in _bal.getPtoMeasurementData().Rows)
            {
                if (Convert.ToDateTime(dt["CreateDate"].ToString()).ToString("dd/MM/yyyy") == DateTime.Now.ToString("dd/MM/yyyy"))
                {
                    ...
                    dgv_PTOMesurement.Rows[i].Cells["ccd_value_"].Value = dt["ccd_value"];
        ...
        private void txt_dailgaugereading_driven_page2_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (txt_BH_From_OC_To_IR_page2.Text != "" && txt_spline_Shaft_Etched_Value_page2.Text != "")
                {
                    txt_shimsRequired_page2.Text = _objValidation.Gettingdecimals((Convert.ToDouble(txt_CCDValue_page2.Text) - (Convert.ToDouble(txt_BH_From_OC_To_IR_page2.Text) + Convert.ToDouble(txt_spline_Shaft_Etched_Value_page2.Text))).ToString());

                    int count0point5microm = 0;
                    int count1point0microm = 0;
                    int count1point1microm = 0;
                    int count1point2microm = 0;
                    int count1point3microm = 0;
                    int count1point4microm = 0;
                    ...
                    string MicroM0 = "1 mm :";
                    string MicroM1 = "1.1 mm :";
                    string MicroM2 = "1.2 mm :";
                    string MicroM3 = "1.3 mm :";
                    string MicroM4 = "1.4 mm :";
                    string MicroM5 = "0.5 mm :";

                    LB_Total_shims.Items.Clear();

                    if (count1point0microm != 0)
                        LB_Total_shims.Items.Add(MicroM0 + count1point0microm.ToString());
                    if (count1point1microm != 0)
                        LB_Total_shims.Items.Add(MicroM1 + count1point1microm.ToString());
                    if (count1point2microm != 0)
                        LB_Total_shims.Items.Add(MicroM2 + count1point2microm.ToString());
                    if (count1point3microm != 0)
                        LB_Total_shims.Items.Add(MicroM3 + count1point3microm.ToString());
                    if (count1point4microm != 0)
                        LB_Total_shims.Items.Add(MicroM4 + count1point4microm.ToString());
                    if (count0point5microm != 0)
                        LB_Total_shims.Items.Add(MicroM5 + count0point5microm.ToString());
        ...
        private void tab_shimmeasurement_Enter(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                txt_spline_Shaft_Etched_Value_page2.Focus();
            }
        }
        ...
        private void tab_shimVerification_Enter(object sender, EventArgs e)
        {
            ...
            {
                bttn_exportexcel.Visible = false;
                txt_BH_From_OC_To_IR_page3.Text = txt_BH_From_OC_To_IR_page2.Text;
                txt_shimsRequiredDrivenVerify.Text = txt_shimsRequired_page2.Text;
        ...
        private void txt_dailgaugereading_driven_page1_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    txt_CCDValue_page2.Text = CCD_Value; // 65.6
            ...
            if (e.KeyData == Keys.F2)
            {
                if (PtoMeasurement.SelectedTab == MasterTab)
                {
                    if (ValidFirst())
                    {
                        ...
                        txt_BH_From_OC_To_IR_page2.Text = txt_masterValue_page1.Text;
                        txt_spline_Shaft_Etched_Value_page2.Focus();
                    }
                }
        ...
        private void cmb_selectmodel_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            CCD_Value = _objValidation.Gettingdecimals(dtmodel.Rows[0]["CCD_Value"].ToString());
        ...
        private void txt_Dial_Gauge_Reading_of_Y_page2_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (txt_BH_From_OC_To_IR_page2.Text != "" && txt_spline_Shaft_Etched_Value_page2.Text != "")
                {
                    txt_shimsRequired_page2.Text = _objValidation.Gettingdecimals(((Convert.ToDouble(txt_BH_From_OC_To_IR_page2.Text) - Convert.ToDouble(txt_spline_Shaft_Etched_Value_page2.Text)) - (0.2)).ToString());
        ...
        private void txt_Dial_Gauge_Reading_of_X_page2_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (txt_spline_Shaft_Etched_Value_page2.Text == "")
                {
                    return;
                }
                else
        ...
        private void bttn_next_page2_Click(object sender, EventArgs e)
        {
            if (ValidSecond())
            {
                ...
                txt_spline_Shaft_Etched_Value_page3.Text = txt_spline_Shaft_Etched_Value_page2.Text;
                txt_BH_From_OC_To_IR_page3.Text = txt_BH_From_OC_To_IR_page2.Text;
                txt_shimsRequiredDrivenVerify.Text = txt_shimsRequired_page2.Text;
        ...
        private void txt_dailgaugereading_page2_TextChanged(object sender, EventArgs e)
        {
            try
            {
                txt_masterValue_page2.Text = "4.512";

                if (txt_masterValue_page2.Text != "" && txt_dailgaugereading_page2.Text != "")
                { 
                    txt_ActualReading_page2.Text = Convert.ToString(Convert.ToDouble(txt_masterValue_page2.Text) - (Convert.ToDouble(txt_dailgaugereading_page2.Text)));
                    txt_BH_From_OC_To_IR_page2.Text = txt_ActualReading_page2.Text;
                }
            }
            catch
            {
